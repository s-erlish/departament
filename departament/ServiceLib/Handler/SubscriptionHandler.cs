...
public static class SubscriptionHandler
{
    ...
    /// <summary>Product token of <see cref="Global.SubscriptionUserAgent"/>, the part before «/».</summary>
    private static readonly string MobileClientProduct = Global.SubscriptionUserAgent.Split('/')[0];
    ...
    /// mobile client (<see cref="Global.SubscriptionUserAgent"/>); any other UA (a browser/branding UA,
    /// or the desktop's own "departament/&lt;version&gt;") is answered with an "app not supported"
    /// placeholder — the panel returns a single fake VLESS node named «Приложение не поддерживается».
    /// This affects BOTH account-imported subs AND a user's OWN manually-added subscription (which
    /// arrives with a blank UA), so we force the SAME mobile-client UA the working account path uses
    /// for every fetch. An item that already carries an explicit mobile-client UA is honoured
    /// verbatim. 1:1 with the Android client (HttpUtil.getUrlContentWithUserAgentEx).
    /// </summary>
    private static string ResolveSubUserAgent(string? userAgent)
    {
        var ua = userAgent?.Trim();
        if (ua.IsNotEmpty() && ua!.Contains(MobileClientProduct, StringComparison.OrdinalIgnoreCase))
        {
            return ua;
        }
