...
public class DownloadService
{
    ...
    /// request to match a real Android client (HttpURLConnection / OkHttp):
    ///   - UA attached raw/unvalidated so the exact literal (<see cref="Global.SubscriptionUserAgent"/>) goes out verbatim,
    ///     as a SINGLE clean header (no default "departament/&lt;ver&gt;" UA is ever appended);
    ...
    private static HttpClient CreateSubscriptionClient(string url, IWebProxy? webProxy, ref string userAgent, int connectTimeout)
    {
        var handler = new SocketsHttpHandler
        {
            ...
            // Advertise + transparently decode exactly "gzip" — byte-identical to a real Android
