...
public class Global
{
    ...
    /// server list only to a recognised mobile client, so this is the Android client's User-Agent.
    /// Single source of truth shared by BOTH the manual subscription-add path
    /// (SubscriptionHandler.ResolveSubUserAgent) and the Telegram/account path
    /// (SubscriptionSyncManager) so the two can never drift.
    /// </summary>
    ...
    public const string InnerUriProtocol = "departament://";
    ...
    public const string AutoRunName = "departamentAutoRun";
    ...
    public const string LocalAppData = "DEPARTAMENT_LOCAL_APPLICATION_DATA";
    ...
    public static readonly Dictionary<ECoreType, string> CoreUrls = new()
    {
        ...
        // Выпуски самого departament для ПК (не апстрим): см. AppUpdateChannel.
        { ECoreType.departament, "s-erlish/departament" },
    };
