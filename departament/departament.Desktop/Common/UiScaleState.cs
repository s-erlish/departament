...
public static class UiScaleState
{
    ...
    /// <summary>
    /// Клампит любой ввод (в т.ч. 0 / NaN из старого или битого конфига) в допустимый диапазон
    /// И ОКРУГЛЯЕТ ЕГО ДО СОТЫХ.
    ///
    /// <para>Округление здесь не косметика. Шаг горячих клавиш <see cref="Step"/> = 0.1 в двоичной
    /// дроби не представим точно, поэтому сложение накапливает ошибку: пять нажатий Ctrl + от 100%
    /// дают не 1.5, а 1.5000000000000004, а Ctrl − четырежды и Ctrl + один раз — 0.7999999999999999
    /// вместо 0.8. Такое значение не совпадает НИ С ОДНИМ пресетом набора, и строка настроек честно
    /// достраивает им список (MergeOption) — из-за чего окошко «Масштаб интерфейса» показывало
    /// «150 %» ДВАЖДЫ, причём галочка стояла на втором. Округление до сотых точно накрывает и шаг
    /// клавиш (0.1), и все пресеты набора (0.75 … 1.5), поэтому дубликату взяться неоткуда, а
    /// уже сохранённое кривое значение чинится на первом же чтении конфига.</para>
    /// </summary>
    public static double Clamp(double value)
    {
        ...
        return Math.Clamp(Math.Round(value, 2), Min, Max);
    }
    ...
    public static double ResolveAuto(double screenPixelWidth, double screenPixelHeight, double osScaling)
    {
        ...
        //  ЯВНАЯ ТАБЛИЦА, а не растущая пропорция. Пропорция давала 1.60 на 2560×1440, и интерфейс
        //  выходил заметно крупнее соседних приложений: замер по кадру владельца — строка списка у
        //  Happ 60 физических пикселей, у нас при 1.60 те же 60 логических превращались в 96.
        //  Владелец: «подогнать размер под Happ». При этом на 1920×1080 он просил именно 1.20,
        //  поэтому кривая почти плоская: плотность держится около выбранных им 120%, а расти масштабу
        //  разрешено только там, где без него мелко физически — на 4K и выше.
        //  Ступени по ВЫСОТЕ в аппаратно-независимых пикселях (ширина в ультрашироких мониторах
        //  обманывает: 3440×1440 — это тот же 1440p по плотности).
        var scale = dipHeight switch
        {
            < 1000 => 1.00,   //  1366×768 и мельче — масштабировать нечего
            < 1200 => 1.20,   //  1920×1080
            < 1700 => 1.25,   //  2560×1440 и 3440×1440
            < 2200 => 1.50,   //  3840×2160
            _ => 1.75,        //  выше 4K
        };
        return Math.Clamp(scale, AutoMin, AutoMax);
    }
