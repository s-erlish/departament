...
using departament.Desktop.Account.Dto;

namespace departament.Desktop.Account;

/// <summary>
/// Bridges the backend subscription payloads into the app's EXISTING subscription plumbing. Port of
/// the Android app's auth/SubscriptionSyncManager.kt — it REUSES the engine rather than duplicating it:
...
///    the mobile-client User-Agent (the panel keys the "real server list vs app-not-supported" response
...
{
    /// <summary>
    /// Mobile-client User-Agent stamped on every account-imported subscription. The departament
    /// panel keys its managed server list off a recognised mobile client, so this must stay that
    /// client's UA — it is passed through verbatim by <see cref="SubscriptionHandler"/>. Single source
    ...
    private async Task<List<string>> Import(PrimarySubscriptionDto? primary, List<SubInfoDto> all, UserProfileDto? profile, bool authoritative)
    {
        ...
        foreach (var candidate in BuildCandidates(primary, all, profile))
        {
            ...
            item.Enabled = true;
            // Stamp an explicit mobile-client UA: the departament/Remnawave panel serves its managed
            // server list only for a recognised mobile client. SubscriptionHandler.ResolveSubUserAgent
            // honours this verbatim. (Manually-added subs carry no UA and get the same one from
            // ResolveSubUserAgent — see that method.)
            ...
            // FORCES the correct mobile-client User-Agent. This is the whole reason a plain
