namespace departament.Desktop.ViewModels;
...
public class SettingsViewModel : MyReactiveObject
{
    ...
    public SettingsViewModel()
    {
        ...
        departament.Desktop.Common.UiScaleState.Changed += OnUiScaleStateChanged;
    ...
    private void LoadFromConfig()
    {
        ...
        try
        {
            ...
            _uiScaleValues = MergeOption(UiScalePresets, departament.Desktop.Common.UiScaleState.Clamp(_config.UiItem.UiScale));
            ...
            UiScaleIndex = IndexOf(_uiScaleValues, departament.Desktop.Common.UiScaleState.Clamp(_config.UiItem.UiScale));
    ...
    //  Значок прячется и возвращается СРАЗУ, без перезапуска: строка описывает наблюдаемое
    //  состояние, а не намерение на следующий запуск.
    private async Task OnHideTrayIconChanged(bool v)
    {
        ...
        departament.Desktop.App.ApplyTrayIconVisibility(v);
    }
    ...
    private async Task OnAutoStartChanged(bool v)
    {
        ...
        if (Utils.IsWindows())
        {
            departament.Desktop.Common.AutostartHelper.Apply(v);
        }
        else
        {
    ...
    private void ReconcileAutostart()
    {
        ...
        _ = Task.Run(() =>
        {
            try
            {
                ...
                {
                    if (!departament.Desktop.Common.AutostartHelper.IsCurrent())
                    {
                        departament.Desktop.Common.AutostartHelper.Set();
                    }
                }
                else if (departament.Desktop.Common.AutostartHelper.IsEnabled())
                {
    ...
    private void ReconcileSystemPreferences()
    {
        ...
        {
            departament.Desktop.App.ApplyTheme(EffectiveThemeName(_config.UiItem.CurrentTheme), _config.UiItem.BlackTheme);
        }
    ...
    private async Task OnLiteModeChanged(bool v)
    {
        ...
        departament.Desktop.Common.MotionState.SetLite(v);
    }
    ...
    private void OnUiScaleIndexChanged(int index)
    {
        ...
        if (Math.Abs(departament.Desktop.Common.UiScaleState.Current - next) < 0.0001)
        {
            return;
        }
        ...
        departament.Desktop.Common.UiScaleState.Set(next);
    ...
    public async Task SetLook(int look)
    {
        ...
        await ConfigHandler.SaveConfig(_config);
        departament.Desktop.App.ApplyTheme(EffectiveThemeName(theme), mono);
    }
